...
.rf-topbar-title { font-size: 16px; font-weight: 700; letter-spacing: .5px; flex: 1; text-align: center; }
.rf-topbar-left, .rf-topbar-right { display: flex; align-items: center; gap: 2px; min-width: 60px; }
...
.rf-avatar { width: 40px; height: 40px; border-radius: 50%; border: 1px solid #e5e5e5; background: #f0f0f0; flex-shrink: 0; overflow: hidden; display: flex; align-items: center; justify-content: center; }
...
.rf-profile-cover { width: 100%; height: 90px; background: #f0f0f0; overflow: hidden; position: relative; flex-shrink: 0; }
.rf-profile-cover img { width: 100%; height: 100%; object-fit: cover; }
.rf-profile-header { padding: 0 14px 10px; border-bottom: 1px solid #e5e5e5; margin-top: -28px; position: relative; }
.rf-profile-avatar { width: 64px; height: 64px; border-radius: 50%; border: 3px solid #fff; background: #f0f0f0; display: flex; align-items: center; justify-content: center; font-size: 28px; overflow: hidden; margin-bottom: 6px; }
...
.rf-detail-comment-input { padding: 10px 14px; border-top: 1px solid #e5e5e5; display: flex; gap: 8px; align-items: flex-end; flex-shrink: 0; }
...
.rf-comment-row { display: flex; gap: 8px; padding: 10px 14px; border-bottom: 1px solid #f0f0f0; }
...
.rf-comment-reply-btn { background: none; border: none; color: #aaa; font-size: 11px; cursor: pointer; padding: 2px 0; margin-top: 3px; }
.rf-comment-reply-btn:hover { color: #666; }
.rf-reply-area { background: #f8f8f8; border-radius: 8px; margin-top: 4px; padding: 6px 10px; }
.rf-reply-item { font-size: 12px; color: #555; padding: 3px 0; border-bottom: 1px solid #eee; line-height: 1.4; }
.rf-reply-item:last-child { border-bottom: none; }
.rf-reply-item b { color: #000; }
.rf-summon-bar { display: flex; justify-content: center; padding: 10px 14px; border-bottom: 1px solid #e5e5e5; }
...
.rf-loading-overlay { position: absolute; inset: 0; background: rgba(255,255,255,0.75); backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px); display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 999; gap: 12px; }
.rf-loading-spinner { font-size: 48px; animation: rf-spin 1.2s linear infinite; display: inline-block; }
...
4. 有没有出现耽美角色？有就改。`,
  ...
  dataLibrary: [],  // [{id, category, content, enabled}]
}

// ═══════════════════════════════════════════════════════════
// Storage
// ═══════════════════════════════════════════════════════════
const STORAGE_KEYS = ['homePosts','crossPosts','parallelPosts','myPosts','savedPosts','userProfile','worldview','boundWorldbooks','beauty','postSettings','dataLibrary']
...
async function buildContext() {
  ...
  // 资料库
  const libContext = state.dataLibrary.filter(e => e.enabled).map(e => (e.category ? `[${e.category}] ` : '') + e.content).join('\n')
...
⚠️ 只输出JSON，不要其他文字。`
  ...
  let userPrompt = ''
  const jsonFmt = (sameWorld) => `{"posts":[{"authorName":"角色名","authorHandle":"昵称","avatar":"","text":"帖子内容","likes":0,"retweets":0,"worldTag":"作品名","comments":[{"name":"角色名","handle":"昵称","avatar":"","text":"评论","worldTag":"${sameWorld ? '必须与发帖人相同的作品名' : '作品名'}"}]}]}`

  if (mode === 'home') {
    userPrompt = `生成${postCount}条主页帖子。每条帖子选一个角色，内容是角色日常/剧情/小烦恼。⚠️每条帖子恰好${commentPerPost}条评论，评论者必须来自与发帖角色【完全相同的作品】。严格遵守世界观规则。\nJSON格式：${jsonFmt(true)}`
  } else if (mode === 'cross') {
    userPrompt = `生成${postCount}条跨界板块帖子，发帖人来自不同作品。评论区有其他作品角色跨界评论（此板块允许跨作品）。恰好${commentPerPost}条评论。严格遵守世界观规则。\nJSON格式：${jsonFmt(false)}`
  } else {
    userPrompt = `生成${postCount}条平行板块帖子，内容是角色与用户@${userHandle}的恋爱日常，温馨乙女向。⚠️${commentPerPost}条评论，评论者必须来自与发帖角色【相同的作品】。严格遵守世界观规则。\nJSON格式：${jsonFmt(true)}`
  }

  const result = await roche.ai.chat({ messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }], temperature: 0.92 })
  let parsed
  try {
  ...
  return (parsed.posts || []).map(p => ({
    ...
    worldTag: p.worldTag || '',
    comments: (p.comments || []).map(c => ({ name: c.name, handle: c.handle || c.name, avatar: '', text: c.text || '', worldTag: c.worldTag || '', replies: [] })),
  }))
}

async function summonComments(post) {
  const { libContext } = await buildContext()
  const worldTag = post.worldTag || '同一作品'
  const count = state.postSettings?.commentPerPost || 3
  const sys = `你是角色论坛AI。只输出JSON，不要其他文字。${libContext ? `资料库参考：\n${libContext}` : ''}`
  const usr = `以下帖子需要新评论：
作者：${post.authorName}（${worldTag}）
内容：${post.text}
请生成${count}条来自${worldTag}角色的评论，禁止BL角色和游戏主角。
JSON格式：{"comments":[{"name":"角色名","handle":"昵称","avatar":"","text":"评论内容","worldTag":"${worldTag}"}]}`
  ...
  return (parsed.comments || []).map(c => ({ name: c.name, handle: c.handle || c.name, avatar: '', text: c.text || '', worldTag: c.worldTag || worldTag, replies: [] }))
}
...
function pagePostDetail(container, post, listKey, onBack) {
  ...
  summonBtn.title = '召唤角色来评论'
  summonBtn.appendChild(mkSvg(ICONS.summon))
  const rightGroup = el('div', { class: 'rf-topbar-right' })
  rightGroup.appendChild(summonBtn)
  ...
  const sub = el('div', { style: 'display:flex;gap:6px;align-items:center;margin-top:2px' })
  sub.appendChild(el('span', { class: 'rf-post-handle' }, '@' + post.authorHandle))
  if (post.worldTag) sub.appendChild(el('span', { class: 'rf-post-tag' }, post.worldTag))
  dm.appendChild(sub)
  dh.appendChild(dm)
  ...
  function renderComments() {
    ...
    if (!post.comments?.length) {
      ...
      for (const c of post.comments) {
        ...
        // 回复列表
        if (c.replies?.length) {
          const replyArea = el('div', { class: 'rf-reply-area' })
          for (const r of c.replies) {
            const ri = el('div', { class: 'rf-reply-item' })
            ri.innerHTML = `<b>${r.name}：</b>${r.text}`
            replyArea.appendChild(ri)
          }
          cbody.appendChild(replyArea)
        }
        ...
        replyBtn.onclick = () => {
          ...
          const sendR = el('button', { class: 'rf-btn rf-btn-primary rf-btn-sm' }, '发送')
          sendR.onclick = async () => {
          ...
          box.appendChild(inp); box.appendChild(sendR)
  ...
  sendBtn.onclick = async () => {
    ...
    post.comments.push({ name: state.userProfile.handle || '我', handle: state.userProfile.handle || '我', avatar: state.userProfile.avatarUrl || '', text, worldTag: '', replies: [] })
    ta.value = ''; ta.style.height = 'auto'
    await save(listKey)
    renderComments()
    roche.ui.toast('✦ 评论已发送')
  }
...
function buildTopBar(title, leftEl, rightEl) {
  ...
  if (leftEl) left.appendChild(leftEl); else { const ph = el('div', { style: 'width:60px' }); bar.appendChild(ph) }
  if (leftEl) bar.appendChild(left)
  else bar.appendChild(el('div', { style: 'width:60px' }))
...
function buildMainTopBar(title, container, onRefresh) {
  // close | title | [settings, refresh]
  ...
  const settingsBtn = el('button', { class: 'rf-icon-btn' })
  settingsBtn.appendChild(mkSvg(ICONS.settings))
  settingsBtn.onclick = () => pageSettings(container)
  ...
  const p = state.beauty?.topbarPadding ?? 10
  const bar = el('div', { class: 'rf-topbar', style: `padding:${p}px 14px` })
  const leftWrap = el('div', { class: 'rf-topbar-left' }); leftWrap.appendChild(closeBtn)
  bar.appendChild(leftWrap)
  bar.appendChild(el('span', { class: 'rf-topbar-title' }, title))
  bar.appendChild(rightGroup)
  return { bar, refreshBtn }
}

function buildBackTopBar(title, onBack, rightEl) {
  const backBtn = el('button', { class: 'rf-icon-btn' })
...
function pageHome(container) {
  ...
  const { bar, refreshBtn } = buildMainTopBar('✦ 论坛', container, null)
  ...
  fab.onclick = () => {
    ...
    postBtn.onclick = async () => {
      const text = ta.value.trim()
      if (!text) return
      const post = { id: crypto.randomUUID(), timestamp: Date.now(), authorName: state.userProfile.handle || '我', authorHandle: state.userProfile.handle || '我', avatar: state.userProfile.avatarUrl || '', text, likes: 0, retweets: 0, worldTag: '', comments: [], isUserPost: true }
...
function pageSection(container) {
  const wrap = el('div', { class: 'rf', style: 'position:relative' })
  const { bar, refreshBtn } = buildMainTopBar('✦ 板块', container, null)
  wrap.appendChild(bar)
...
function pageMessage(container) {
  ...
  closeBtn.appendChild(mkSvg(ICONS.close))
  wrap.appendChild(buildTopBar('✦ 私信', closeBtn, null))
  const content = el('div', { class: 'rf-content' })
...
function pageProfile(container) {
  ...
  const closeBtn = el('button', { class: 'rf-icon-btn', onclick: () => roche.ui.closeApp() })
  closeBtn.appendChild(mkSvg(ICONS.close))
  const leftGroup = el('div', { class: 'rf-topbar-left' }); leftGroup.appendChild(closeBtn)
  const rightGroup = el('div', { class: 'rf-topbar-right' }); rightGroup.appendChild(settingsBtn)
  const p = state.beauty?.topbarPadding ?? 10
  const bar = el('div', { class: 'rf-topbar', style: `padding:${p}px 14px` })
  bar.appendChild(leftGroup)
  bar.appendChild(el('span', { class: 'rf-topbar-title' }, '✦ 我的'))
  bar.appendChild(rightGroup)
  wrap.appendChild(bar)
  ...
  if (state.userProfile.coverUrl) {
    const img = el('img', { src: state.userProfile.coverUrl, alt: '' })
    img.onerror = () => img.remove()
    coverEl.appendChild(img)
  } else {
    coverEl.style.background = 'linear-gradient(135deg,#e5e5e5,#d0d0d0)'
  }
  ...
  function drawContent() {
    ...
    if (state.profileTab === 0) {
      ...
      else for (const p of state.myPosts) content.appendChild(postCard(p, 'myPosts', post => openDetail(post, 'myPosts')))
      ...
      content.appendChild(ew)
    } else if (state.profileTab === 2) {
      if (!state.savedPosts.length) content.appendChild(el('div', { class: 'rf-empty' }, '还没有收藏任何帖子'))
      else for (const p of state.savedPosts) content.appendChild(postCard(p, 'savedPosts', post => openDetail(post, 'savedPosts')))
    } else {
      const wip = el('div', { class: 'rf-wip' })
...
function pagePostSettings(container) {
  ...
  const pcLabel = el('div', { class: 'rf-beauty-label' })
  pcLabel.appendChild(document.createTextNode('每次生成帖子数'))
  pcLabel.appendChild(pcVal)
  ...
  pcGroup.appendChild(pcSlider)
  content.appendChild(pcGroup)

  // Comment per post slider
  const ccGroup = el('div', { class: 'rf-beauty-group' })
  const ccVal = el('span', {}, (state.postSettings?.commentPerPost || 3) + ' 条')
  const ccLabel = el('div', { class: 'rf-beauty-label' })
  ccLabel.appendChild(document.createTextNode('每条帖子评论数'))
  ccLabel.appendChild(ccVal)
  ...
  ccGroup.appendChild(ccSlider)
  content.appendChild(ccGroup)

  content.appendChild(el('div', { style: 'font-size:11px;color:#aaa;padding:10px 16px;line-height:1.6' }, '提示：数量越多，AI生成时间越长。建议帖子5条、评论3条。'))
  wrap.appendChild(content)

  saveBtn.onclick = async () => {
    state.postSettings = { postCount: parseInt(pcSlider.value), commentPerPost: parseInt(ccSlider.value) }
    await save('postSettings')
    roche.ui.toast('✦ 帖子设置已保存')
...
function pageDataLibrary(container) {
  ...
  function renderLib() {
    content.innerHTML = ''
    // 总开关提示
    const hint = el('div', { style: 'font-size:11px;color:#aaa;padding:10px 16px;line-height:1.6;border-bottom:1px solid #f0f0f0' }, '勾选的条款将在刷新帖子时作为参考提供给AI，可按分类整理角色/设定等信息。')
    content.appendChild(hint)

    if (!state.dataLibrary.length) {
      content.appendChild(el('div', { class: 'rf-empty' }, '暂无资料\n点击右上角 + 添加'))
    } else {
      for (let i = 0; i < state.dataLibrary.length; i++) {
        ...
        delBtn.onclick = async () => {
          const ok = await roche.ui.confirm({ title: '删除条款', message: '确定删除这条资料吗？' })
          if (!ok) return
          state.dataLibrary.splice(i, 1)
          await save('dataLibrary')
          renderLib()
        }
  ...
  addBtn.onclick = () => {
    ...
    const catGroup = el('div', { style: 'margin-bottom:10px' })
    catGroup.appendChild(el('div', { class: 'rf-form-label' }, '分类（选填，如：角色、设定、禁忌）'))
    ...
    const contentGroup = el('div', { style: 'margin-bottom:10px' })
    contentGroup.appendChild(el('div', { class: 'rf-form-label' }, '条款内容（提供给AI的参考文字）'))
    ...
    confBtn.onclick = async () => {
      const content2 = ta.value.trim()
      if (!content2) return
      state.dataLibrary.push({ id: crypto.randomUUID(), category: catInput.value.trim(), content: content2, enabled: true })
      await save('dataLibrary')
      overlay.remove()
      renderLib()
      roche.ui.toast('✦ 已添加')
...
function pageAvatar(container) {
  ...
  const coverPreview = el('div', { style: 'width:100%;height:80px;background:#f0f0f0;overflow:hidden;position:relative' })
  if (state.userProfile.coverUrl) {
    const img = el('img', { src: state.userProfile.coverUrl, alt: '', style: 'width:100%;height:100%;object-fit:cover' })
    img.onerror = () => img.remove()
    coverPreview.appendChild(img)
  }
  content.appendChild(coverPreview)

  const cg1 = el('div', { class: 'rf-form-group' })
  cg1.appendChild(el('div', { class: 'rf-form-label' }, '封面图片 URL'))
  const coverInput = el('input', { class: 'rf-form-input', type: 'url', placeholder: 'https://…' })
  coverInput.value = state.userProfile.coverUrl || ''
  cg1.appendChild(coverInput)
  content.appendChild(cg1)

  const cg2 = el('div', { class: 'rf-form-group' })
  cg2.appendChild(el('div', { class: 'rf-form-label' }, '或从相册选择封面'))
  const coverFile = el('input', { type: 'file', accept: 'image/*', style: 'margin-top:6px' })
  coverFile.onchange = e => {
    const file = e.target.files[0]; if (!file) return
    const reader = new FileReader()
    reader.onload = ev => { coverInput.value = ev.target.result; coverPreview.innerHTML = ''; coverPreview.appendChild(el('img', { src: ev.target.result, alt: '', style: 'width:100%;height:100%;object-fit:cover' })) }
    reader.readAsDataURL(file)
  }
  cg2.appendChild(coverFile)
  ...
  avPreview.appendChild(avCircle)
  content.appendChild(avPreview)

  const ag1 = el('div', { class: 'rf-form-group' })
  ag1.appendChild(el('div', { class: 'rf-form-label' }, '头像 URL'))
  const urlInput = el('input', { class: 'rf-form-input', type: 'url', placeholder: 'https://…' })
  urlInput.value = state.userProfile.avatarUrl || ''
  ag1.appendChild(urlInput)
  content.appendChild(ag1)

  const ag2 = el('div', { class: 'rf-form-group' })
  ag2.appendChild(el('div', { class: 'rf-form-label' }, '或从相册选择头像'))
  const fileInput = el('input', { type: 'file', accept: 'image/*', style: 'margin-top:6px' })
  fileInput.onchange = e => {
    const file = e.target.files[0]; if (!file) return
    const reader = new FileReader()
    reader.onload = ev => { urlInput.value = ev.target.result; avCircle.innerHTML = ''; avCircle.appendChild(el('img', { src: ev.target.result, alt: '' })) }
    reader.readAsDataURL(file)
  }
  ag2.appendChild(fileInput)
  content.appendChild(ag2)
...
function pageProfileEdit(container) {
  ...
  for (const f of fields) {
    ...
    inputs[f.key] = inp; grp.appendChild(inp); content.appendChild(grp)
  }
  wrap.appendChild(content)
  saveBtn.onclick = async () => {
...
function pageBeauty(container) {
  ...
  const padLabel = el('div', { class: 'rf-beauty-label' })
  padLabel.appendChild(document.createTextNode('顶部栏上下间距'))
  padLabel.appendChild(padVal)
  padGroup.appendChild(padLabel)
  const padSlider = el('input', { class: 'rf-beauty-slider', type: 'range', min: '4', max: '22', step: '1' })
  padSlider.value = state.beauty?.topbarPadding ?? 10
  padSlider.oninput = () => { padVal.textContent = padSlider.value + 'px' }
  padGroup.appendChild(padSlider)
  content.appendChild(padGroup)
  wrap.appendChild(content)
